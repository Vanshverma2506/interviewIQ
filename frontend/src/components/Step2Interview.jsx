...
const Step2Interview = ({ interviewData, onFinish }) => {
  ...
  const speakText = (text, isQuestion = false) => {
  ...
  useEffect(() => {
    ...
    const runIntro = async () => {
      ...
      } else if (currentQuestion && !hasSpokenRef.current) {
        ...
        <div className="flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative">
          ...
          {feedback === "" ? (
            ...
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="mt-6 bg-emerald-50 border-emerald-200 p-5 rounded-2xl shadow-sm "
            >
